...
class ItemManager(models.Manager):
    def create_item(self, data):
        user_obj = Profile.objects.get(id=data['seller']['id'])
        try:
            created_item = self.create(item_name=data['item_name'], price=data['price'], views=1, description=data['description'], seller=user_obj, quantity=data['quantity'])
            created_item = model_to_dict(created_item)
...
class TagManager(models.Manager):
    def get_tag_list(self, blank=False):
